...
Created on Thu Dec 12 18:51:00 2019
...
extreme radiation, and a whole lot of moons swirling around. You decide to start by tracking the
...
Then, once all moons' velocities have been updated, update

the position of every moon by applying velocity. Time progresses by one step
...
However, if the positions on a given axis are the same, the velocity
...
of each moon to its own position. For example, if Europa has a

position of x=1, y=2, z=3 and a velocity of x=-2, y=0,z=3,

then its new position would be x=-1, y=2, z=6. This process

does not modify the velocity of any moon.
...
Below, each line shows the calculations for a moon's potential energy (pot),
...
In the above example, adding together the total energy

for all moons after 10 steps produces the total energy in the system, 179.
...
scan = [[3, -6, 6,0,0,0],[10, 7, -9,0,0,0],[-3, -7, 9,0,0,0],[-8, 0, 4,0,0,0]]

for i in range(1000):
    scan = upPos(upVel(scan))
...
Does history really repeat itself? You're curious whether the moons will ever 
...
a previous state! Clearly, you might need to find a more efficient 
way to simulate the universe.
...
#Computes periods for x, y, z axes; LCM computed on line
#https://www.calculatorsoup.com/calculators/math/lcm.php
def getPeriod(indx):
    scan = [[3, -6, 6,0,0,0],[10, 7, -9,0,0,0],[-3, -7, 9,0,0,0],[-8, 0, 4,0,0,0]]
    perm = [[3, -6, 6,0,0,0],[10, 7, -9,0,0,0],[-3, -7, 9,0,0,0],[-8, 0, 4,0,0,0]]

#    scan = [[-1, 0,2,0, 0,0], [ 2,-10, -7, 0, 0, 0], [ 4,-8, 8, 0, 0, 0],[ 3, 5, -1,0,0,0]]
#    perm = [[-1, 0,2,0, 0,0], [ 2,-10, -7, 0, 0, 0], [ 4,-8, 8, 0, 0, 0],[ 3, 5, -1,0,0,0]]
    for i in range(1,300000):
        scan = upPos(upVel(scan))
        if (scan[0][indx] == perm[0][indx] and \
            scan[1][indx] == perm[1][indx]and \
            scan[2][indx] == perm[2][indx]and \
            scan[0][indx+3] == perm[0][indx+3]and \
            scan[1][indx+3] == perm[1][indx+3]and \
            scan[2][indx+3] == perm[2][indx+3]):
            return i
for k in range(0,3):
    print(getPeriod(k)) 
