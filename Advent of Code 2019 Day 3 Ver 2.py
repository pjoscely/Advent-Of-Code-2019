...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec  6 21:44:08 2019
...
Created on Tue Dec  3 18:45:15 2019
...
to the central port. Because the wires are on a grid, use the Manhattan distance
...
each intersection; choose the intersection where the sum of both wires'
 steps is lowest. If a wire visits a position on the grid 
 multiple times, use the steps value from the first time 
 it visits that position when calculating the total value 
 of a specific intersection.

The number of steps a wire takes is the total number of grid squares
 the wire has entered to get to that location, including the intersection being considered. Again consider the example from above:
...
#Input data
A=open('/Users/joscelynec/Desktop/Day3A.txt').read().split(',')
B=open('/Users/joscelynec/Desktop/Day3B.txt').read().split(',')

DX = {'L':-1,'R':1,'D':0,'U':0}
DY = {'L':0,'R':0,'D':-1,'U':1}
#Converts input code to a lattice path
def get_points(A):
    x = 0
    y = 0
    length = 0
    ans= {} #Dictionary containing (x,y):length form the origin
    for cmd in A:
        d = cmd[0]
        n = int(cmd[1:])
        assert d in ['L','R','U','D']
